...
class VariablesTestCase(tests.TestCase):
    ...
    def test_local_var(self):
        tree, builder = ast.parse("local foo = 42")
    ...
    def test_empty_local_var(self):
        tree, builder = ast.parse("local foo, bar")
        exp = Chunk(Block([LocalAssign(
    ...
    def test_multi_var(self):
        tree, builder = ast.parse("a,b,c = 1,2")
        exp = Chunk(Block([Assign(
            targets=[
                Name('a'),
                Name('b'),
    ...
    def test_local_multi_var(self):
        tree, builder = ast.parse("local foo, bar = 42")
        exp = Chunk(Block([LocalAssign(
            targets=[
                Name('foo'),
                Name('bar')
